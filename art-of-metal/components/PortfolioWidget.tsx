...
const data = [
  ...
  { name: "Other", value: 3.6, color: "#2A2A2A" },
];

export default function PortfolioWidget({ compact = false }: { compact?: boolean }) {
  if (compact) {
    return (
      <div>
        <p style={{ color: "#555", fontSize: "0.6rem", fontWeight: 600, letterSpacing: "0.1em", marginBottom: 10 }}>PORTFOLIO MIX</p>
        <div style={{ height: 90 }}>
          ...
          {data.map((d, i) => (
            <div key={i} style={{ display: "flex", justifyContent: "space-between" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                ...
                <span style={{ color: "#555", fontSize: "0.6rem" }}>{d.name}</span>
              </div>
              <span style={{ color: "#777", fontSize: "0.6rem", fontWeight: 600 }}>{d.value}%</span>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div style={{ background: "#0A0A0A", border: "1px solid #1A1A1A", borderRadius: 14, padding: 20 }}>
      ...
      <p className="gold-text" style={{ fontSize: "1.8rem", fontWeight: 900, lineHeight: 1, marginBottom: 4 }}>$127,842.36</p>
